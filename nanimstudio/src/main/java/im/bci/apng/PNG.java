 ...
 *           
 * 14/08/13: Add APNG support by devnewton!          
 *
 * Terms of Use:  
 *
...
public class PNG {
	...
	public static byte[] createAnimationControlChunk(int nbFrame, int nbPlays) throws IOException {
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		DataOutputStream chunk = new DataOutputStream(baos);
		chunk.writeInt(nbFrame);
		chunk.writeInt(nbPlays);
		return toChunk("acTL", baos.toByteArray());
	}
	...
	public static byte[] createFrameControlChunk(int sequenceNumber, int width, int height, int xOffset, int yOffset, int delayInMilliseconds, DisposeOp disposeOp, BlendOp blendOp) throws IOException {
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		DataOutputStream chunk = new DataOutputStream(baos);
		chunk.writeInt(sequenceNumber);
		chunk.writeInt(width);
		chunk.writeInt(height);
		chunk.writeInt(xOffset);
		chunk.writeInt(yOffset);
		chunk.writeShort(delayInMilliseconds);
		chunk.writeShort(1000);
		chunk.writeByte(disposeOp.ordinal());
		chunk.writeByte(blendOp.ordinal());
		return toChunk("fcTL", baos.toByteArray());
	}
	
	public static byte[] createDataChunk(int width, int height, byte[] rgba) throws IOException { 
	    return toChunk("IDAT", encodeImage(width, height, rgba));
	}
	
	public static byte[] createFrameDataChunk(int sequenceNumber, int width, int height, byte[] rgba) throws IOException {
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		DataOutputStream chunk = new DataOutputStream(baos);
		chunk.writeInt(sequenceNumber);
		chunk.write(encodeImage(width, height, rgba));
		return toChunk("fdAT", baos.toByteArray());
	}
	
	private static byte[] encodeImage(int width, int height, byte[] rgba) throws IOException {
	    int source = 0;
	    int dest = 0;
